...
import api from "@/lib/api"; // Updated to import your new Axios instance
...
const EXPERIENCE_LEVELS: {
  ...
  { value: "0-1", label: "0–1 years", helper: "New to the field" },
  { value: "1-2", label: "1–2 years", helper: "Early career" },
  { value: "2-5", label: "2–5 years", helper: "Mid-level" },
  { value: "5+", label: "5+ years", helper: "Senior" },
];
...
const INTERVIEW_TYPES: {
  ...
  },
  {
    value: "behavioral",
    label: "Behavioral",
    description: "Focus on communication, teamwork and workplace situations.",
...
const DIFFICULTIES: {
  ...
  },
  {
    value: "medium",
    label: "Medium",
    description: "Practical and moderately challenging questions.",
    icon: <GaugeCircle size={17} strokeWidth={1.75} />,
  },
  {
    value: "hard",
    label: "Hard",
    description: "Advanced concepts, problem-solving and deeper technical questions.",
...
const DURATIONS: {
  ...
  { value: 15, label: "15 min", helper: "Quick practice round" },
  { value: 30, label: "30 min", helper: "Standard interview" },
  { value: 45, label: "45 min", helper: "In-depth session" },
];
  ...
  return (
    <button
      ...
      className={[
        ...
        layout === "compact" ? "px-4 py-3" : "px-4 py-4 sm:px-5 sm:py-4",
        selected
...
function ResumeUploader({
  ...
  const handleFiles = async (files: FileList | null) => {
    ...
    try {
      ...
      // Axios implementation: Tokens and Content-Type are handled automatically
      ...
      alert(errorData?.detail || errorData?.file?.[0] || "Failed to upload resume.");
    }
  };
          ...
          <div className="min-w-0">
            ...
            <p className="text-xs text-ink-muted">
              ...
              <span className="text-brand-600">
                {resume.status === "uploaded" ? " Uploaded" : " Selected"}
              </span>
            </p>
          </div>
    ...
    <div>
      ...
      >
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-ink-muted">PDF, up to 5 MB</p>
        </div>
...
function JobRoleSelect({
  ...
  if (loading) {
    return (
      <div className="flex items-center gap-3 rounded-xl border border-line bg-surface px-4 py-3.5">
        ...
        <span className="text-sm text-ink-muted">Loading job roles...</span>
      </div>
    );
  }
  ...
  if (jobRoles.length === 0) {
    return (
      <div className="rounded-xl border border-line bg-paper px-4 py-4">
        <p className="text-sm text-ink-muted">No job roles are available right now.</p>
      </div>
    );
  }
    ...
    <div ref={containerRef} className="relative">
      <button
        ...
        className={[
          ...
          open ? "border-brand-500 shadow-focus" : "border-line hover:border-ink-faint",
        ].join(" ")}
      >
              ...
              <li key={role.id}>
                <button
                  ...
                  className={[
                    ...
                    selected ? "bg-brand-50 font-medium text-brand-700" : "text-ink hover:bg-paper",
                  ].join(" ")}
                >
                  {role.name}
...
function InterviewSummary({ config }: { config: InterviewConfig }) {
  const experienceLabel = EXPERIENCE_LEVELS.find((e) => e.value === config.experienceLevel)?.label;
  const typeLabel = INTERVIEW_TYPES.find((t) => t.value === config.interviewType)?.label;
  const difficultyLabel = DIFFICULTIES.find((d) => d.value === config.difficulty)?.label;
  const durationLabel = DURATIONS.find((d) => d.value === config.duration)?.label;

  return (
    <div className="rounded-2xl border border-line bg-surface p-5 shadow-card sm:p-6">
      <h2 className="text-base font-semibold text-ink">Interview Summary</h2>
      <p className="mt-1 text-sm text-ink-muted">Review your settings before you begin.</p>
        ...
        <div className="flex items-center gap-3">
          <span
            className={[
              ...
              config.resume ? "bg-brand-600/10 text-brand-600" : "bg-line-soft text-ink-faint",
            ].join(" ")}
          >
            {config.resume ? <FileText size={17} strokeWidth={1.75} /> : <FileWarning size={17} strokeWidth={1.75} />}
          </span>
          <div className="min-w-0">
            <p className="text-xs text-ink-muted">Resume</p>
            <p className="truncate text-sm font-medium text-ink">
              {config.resume ? config.resume.name : "Not uploaded yet"}
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="mt-2 divide-y divide-line-soft">
        <Row label="Job Role" value={config.jobRole?.name ?? "—"} />
        <Row label="Experience" value={experienceLabel ?? "—"} />
        <Row label="Interview" value={typeLabel ?? "—"} />
        <Row label="Difficulty" value={difficultyLabel ?? "—"} />
...
export default function InterviewSetupPage() {
  ...
  const fetchJobRoles = async () => {
    try {
      ...
      const response = await api.get("/api/interviews/job-roles/");
      const data = response.data;
      ...
      setJobRoleError(
        error.response?.data?.detail || "Something went wrong while loading job roles."
      );
    } finally {
  ...
  const handleStartInterview = async () => {
    ...
    if (!config.resume.id) {
      alert("Please upload/select your resume from your account.");
      return;
    }
    ...
    try {
      ...
      alert(`Interview started successfully! Interview ID: ${data.interview_id}`);
      ...
      const errorData = error.response?.data;
      alert(errorData?.detail || "Something went wrong while starting the interview.");
    } finally {
      setStartingInterview(false);
    }
  };

  const canStart = Boolean(config.resume) && Boolean(config.jobRole) && !startingInterview;
      ...
      <div className="mx-auto max-w-6xl px-4 py-10 sm:px-6 sm:py-14 lg:px-8">
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_360px] lg:items-start lg:gap-8">
          <div className="flex flex-col gap-6">
            <SetupSection title="Resume" description="Your interview questions will be tailored to your resume.">
              <ResumeUploader resume={config.resume} onChange={(resume) => setConfig((c) => ({ ...c, resume }))} />
            </SetupSection>
            ...
            <SetupSection
              ...
              trailing={
                <span className="hidden h-9 w-9 shrink-0 items-center justify-center rounded-lg bg-brand-600/10 text-brand-600 sm:flex">
                  <IdCard size={17} strokeWidth={1.75} />
                </span>
              }
            >
              <JobRoleSelect
                ...
                onChange={(jobRole) => setConfig((c) => ({ ...c, jobRole }))}
              />
            </SetupSection>

            <SetupSection title="Experience Level" description="How much experience do you have in this role?">
              <div role="radiogroup" aria-label="Experience level" className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                {EXPERIENCE_LEVELS.map((level) => (
                  <SelectionCard
                    key={level.value}
                    title={level.label}
                    helper={level.helper}
                    selected={config.experienceLevel === level.value}
                    onSelect={() => setConfig((c) => ({ ...c, experienceLevel: level.value }))}
                    layout="compact"
                  />
                ))}
              </div>
            ...
            <SetupSection title="Interview Type" description="Choose what this session should focus on.">
              <div role="radiogroup" aria-label="Interview type" className="grid gap-3 sm:grid-cols-3">
                {INTERVIEW_TYPES.map((type) => (
                  <SelectionCard
                    ...
                    icon={type.icon}
                    selected={config.interviewType === type.value}
                    onSelect={() => setConfig((c) => ({ ...c, interviewType: type.value }))}
                  />
                ))}
              </div>
            </SetupSection>

            <SetupSection title="Difficulty" description="Set how challenging the questions should be.">
              <div role="radiogroup" aria-label="Difficulty" className="grid gap-3 sm:grid-cols-3">
                {DIFFICULTIES.map((difficulty) => (
                  <SelectionCard
                    ...
                    selected={config.difficulty === difficulty.value}
                    onSelect={() => setConfig((c) => ({ ...c, difficulty: difficulty.value }))}
                  />
                ))}
              </div>
            </SetupSection>

            <SetupSection title="Interview Duration" description="Choose how long your session will run.">
              <div role="radiogroup" aria-label="Interview duration" className="grid grid-cols-3 gap-3">
                {DURATIONS.map((duration) => (
                  <SelectionCard
                    ...
                    selected={config.duration === duration.value}
                    onSelect={() => setConfig((c) => ({ ...c, duration: duration.value }))}
                  />
                ))}
              </div>
            </SetupSection>
          ...
          <div className="lg:sticky lg:top-8">
            <div className="flex flex-col gap-4">
              <InterviewSummary config={config} />
              <div className="rounded-2xl border border-line bg-surface p-5 shadow-card sm:p-6">
                <button
                  type="button"
                ...
                >
                  ...
                  {!startingInterview && <ArrowRight size={17} strokeWidth={2} />}
                </button>
